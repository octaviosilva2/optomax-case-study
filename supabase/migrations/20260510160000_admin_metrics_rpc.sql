-- RPC para o painel /admin: retorna métricas agregadas por usuário/org.
-- Substitui 5 queries N+1 do admin/page.tsx por uma única query.
--
-- Permissões: SECURITY DEFINER + restrição: só pode ser chamada via supabaseAdmin
-- (createAdminClient com SERVICE_ROLE_KEY). Cliente normal não tem GRANT.

-- DROP antes de CREATE: replays locais (baseline já tem a versão mais nova da
-- função, com mais colunas) travariam em "cannot change return type of
-- existing function" — Postgres não permite mudar tipo de retorno via OR REPLACE.
...
-- Revoga permissões de roles padrão — só service_role chama
